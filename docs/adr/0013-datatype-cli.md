...
NETAUTO needs a command-line interface for the DataType engine while
preserving the REST-first architecture and avoiding a second execution path
into backend code.
...
Arbitrary-size JSON integers must be preserved. M1.1.10 completes the
DataType Engine CLI surface.

## Consequences

The CLI remains a thin, automatable HTTP client, model-as-code files can be
applied directly against the REST API, and backend rules stay centralized in
the server.
